...
def auto_time_scale(time_array):
    """
    Choose a reasonable time scale and return:
    scaled_time, unit_label
    """
...
def plot_dump(folder: Path, base_name_with_ts: str, channels) -> Path:
    ...
    for ch in channels:
        ...
        t_scaled, unit = auto_time_scale(df["time"].values)
        ax.plot(t_scaled, df["voltage"], label=f"CH{ch}")
